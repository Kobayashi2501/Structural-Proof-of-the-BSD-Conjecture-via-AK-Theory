...
\paragraph{Persistent Barcode (PH₁) Profile.}
For the cyclic orbit \( \{kP\} \subset E(\mathbb{Q}) \), the persistent homology barcode diagram in dimension 1, \( \mathrm{PH}_1 \), exhibits:

\begin{itemize}
  \item One long bar (generator) corresponding to the loop induced by the additive group structure of \( E(\mathbb{Q}) \).
  \item Several short bars indicating transient topological noise.
\end{itemize}

This pattern implies that:
\[
\mathrm{PH}_1(E(\mathbb{Q})) = 0 \quad \text{(after simplification or noise thresholding)}
\]
...
E(\mathbb{Q}) \arrow[r, "\text{Orbit}"] \arrow[d, swap, "PH_1"] &
S_n \subset \mathbb{R}^2 \arrow[d, "VR_\epsilon"] \\
\mathrm{rank}_{\mathbb{Q}}> 0

\arrow[r, Rightarrow, swap, "H_1 \neq 0"] &
\text{Rips complex} 
\arrow[r, "\text{Barcode}"] &
\mathrm{PH}_1(S_n)
\end{tikzcd}
...
\mathcal{E}_{\text{top}}(X) 

\arrow[r, "\text{semantic equivalence}"] &
\mathrm{Ext}^1 = 0 \arrow[r, Rightarrow] &
\Sha(E) = 0
\end{tikzcd}
\]
...
\[
\begin{tikzcd}[row sep=large, column sep=huge]
\mathrm{Ext}^1(F_E, \mathbb{Q}_\ell) = 0 

\arrow[r, Leftrightarrow, "\text{Top collapse}"] \arrow[d, Rightarrow, "\phi_E"] & 
...
\arrow[r, Rightarrow] \arrow[d, "\text{Langlands functor}"'] &
\Sha(E) = 0 \arrow[r, Rightarrow] &
\mathrm{rank}_{\mathbb{Q}} E = \operatorname{ord}_{s=1} L(E,s) \\
...
Categorical & \( \mathrm{Ext}^1 = 0 \) & Formal Lemma (A) \\ (Appendix C.8) \\
Arithmetic & \( \Sha(E) = 0 \) & Formal Lemma (A) \\ (Appendix D.4) \\
...
Type-Theoretic & Collapse via \( \Pi/\Sigma \) Encoding & Formally Described (B+) \\ (Appendix Z.9) \\
\bottomrule
\end{tabular}

\vspace{0.5em}
\textbf{Table.} Proof maturity levels of collapse components in the BSD structural framework.
\end{center}
...
\textbf{A2} & \parbox[t]{10cm}{Persistent homology collapse \( \mathrm{PH}_1 = 0 \) \( \Longleftrightarrow \) Ext-class vanishing \( \mathrm{Ext}^1 = 0 \) \\ \hfill [Appendix B$^+$, G.5, G.7, Y]} \\
...
\begin{center}
\begin{tabular}{p{1.5cm} p{4.8cm} p{4.5cm} p{3.5cm}}
\toprule
...
A2 & \( \mathrm{PH}_1 = 0 \Rightarrow \) local triviality & Isomap + PH collapse & Appendix B, B⁺, Step 1 \\
A3 & \( \mathrm{Ext}^1 = 0 \Rightarrow \) gluing success & Categorical obstruction vanishing & Appendix C.8, G, Step 2 \\
A4 & \( \mathrm{Ext}^1 = 0 \Rightarrow \Sha = 0 \) & Arithmetic sheaf cohomology & Appendix D.4, H, Step 3 \\
A5 & \( \Sha = 0 \Rightarrow \) rank detectability & BSD analytic structure & Appendix I, Step 4 \\
A6 & Collapse chain is functorial across levels & PH–Ext–Sha–Rank integration & Appendix Y, Z.8 \\
A7 & Semantic collapse is type-theoretically trivial & Type-theoretic formalization & Appendix Z.9 \\
\bottomrule
\end{tabular}

\vspace{0.5em}
\textbf{Table.} Collapse axioms and their formal realization across the BSD proof structure.
\end{center}
